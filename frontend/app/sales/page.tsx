                ...
                return (
                  <Dialog key={sale.id}>
                      ...
                      >
                        <TableCell>
                          <code className="text-xs bg-secondary px-2 py-1 rounded">
                            #{sale.id.includes('-') ? sale.id.split('-')[1] : sale.id.substring(0, 8)}
                          </code>
                        </TableCell>
                        <TableCell>
                    ...
                    <DialogContent className="max-w-md bg-card border-border">
                      <DialogHeader>
                        <DialogTitle>Чек #{sale.id.includes('-') ? sale.id.split('-')[1] : sale.id.substring(0, 8)}</DialogTitle>
                      </DialogHeader>
                      <div className="space-y-4 py-4">
                        ...
                        <div className="space-y-2">
                          ...
                          {sale.items.map((item: any, idx: number) => {
                            const itemId = (item.productId || '').includes('-') ? item.productId.split('-')[1] : (item.productId || 'unknown')
                            return (
                              <div key={idx} className="flex justify-between text-sm">
                                <span>Товар #{itemId} x {item.quantity}л</span>
